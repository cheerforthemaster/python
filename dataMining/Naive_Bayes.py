...
def testClassify():
    ...
    # 25个标注为广告的文档
    for i in range(1, 26):
        wordList = textParse(open('email/spam/%d.txt' % i).read(),False)
        listAllDoc.append(wordList)
        listClasses.append(CLASS_AD)
    # 25个标注为非广告的文档
    for i in range(1, 26):
        wordList = textParse(open('email/ham/%d.txt' % i).read(),False)
        listAllDoc.append(wordList)
    ...
    "pClassAD:", pClassAD
    print
    "ad: %d, not ad:%d" % (CLASS_AD, CLASS_NOT_AD)
    ...
    for docIndex in testSet:
        ...
        if classifyNaiveBayes(array(vecWord), pADV, pNotADV, pClassAD) != listClasses[docIndex]:
            ...
            print
            "classfication error", doc.decode('utf-8', "ignore").encode('gbk')
    print
...
'''
    训练
    params:
        tranMatrix 由测试文档转化成的词空间向量所组成的测试矩阵
...
def adClassify(text):
    ...
    print "classfication type is " + Type
...
if __name__ == "__main__":
    ...
    begtime = time.time()
    testClassify()
    print "cost time total:", time.time() - begtime,"s"
